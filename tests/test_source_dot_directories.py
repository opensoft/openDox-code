...
ARRIVED HERE FROM openXdox-code AT § 3.4 SLICE S6, unchanged but for this
paragraph and the `WEB` constant below. RULED Q4 (Brett Heap, 2026-09-12,
`opensoft/openxFactory#656` comment `5642758731`): *"`/source/` is openDox's,
and openXdox's projection binding keeps only `/snapshot-index.json` and the
three `/projections/*` routes."* The route this file is about is a FIXED CORE
ARM of `opendox/serve.py` now, so its suite belongs to this leg — and it is the
one file of the 48 (boundary note § 1.2(d)) whose whole SUBJECT is that route,
which is why it travels with the slice rather than waiting for S8's wholesale
re-homing. Every assertion below is the text openXdox-code carried; the file
already read `opendox.serve` before the move (the carve's `import rewrites`
class had pointed it here), so nothing in it changes meaning by arriving.

IT DOES NOT RUN YET, at this leg or the one it came from — § 1.2(d)'s finding,
and RULED Q-L5 (b') is the narrowing that records it: `conftest` resolves
`REPO_ROOT` at the pre-carve nesting and the harness reaches
`ideation_dashboard`, a package present at neither destination. S6's RUNNABLE
half is `tests/test_source_core_arm.py`, which is on `validate`'s explicit list
and holds the same claims as SHAPE assertions. This file is here so that when
S8 un-narrows the suites it is already at the leg that owns the route.
"""
...
# The bundle is `src/opendox/web/` at this leg — the spelling every other
# openDox-code suite uses (`test_doc_surfaces.py`:39). openXdox-code's copy
# read `src/openxdox/web`, a directory that leg does not have: the carve's
# `import rewrites` class rewrote the constant and pointed it at the wrong
# leg (boundary note § 1.2(d)). Corrected on arrival rather than carried.
