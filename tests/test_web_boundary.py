...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "#8 -> a44ac06d): 18 sites outside class B name a route another column "
        "declares -- 10 the gate prefix (views/lens-model.js:1036/1037, "
        "views/repo-selector.js:39/43, views/staging-workbench-model.js:543/"
        "817/818/821/822/826), 3 openxFactory's adapter lanes "
        "(views/lens.js:41/45, views/repo-selector.js:46), 5 openXdox's "
        "projection routes (views/repo-selector.js:33, app.js:183/184, "
        ...
        "table (point 2) instead reads S4 for 14 of the 18 and S6 for the "
        "remaining 4 -- the two do not fully agree, flagged here rather than "
        "silently reconciled. Unmark only once verified green, whichever "
        "slice(s) actually close it."
    ),
)
...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "(opensoft/openDox-spec #8 -> a44ac06d): 7 sites today -- 2 "
        ...
        "docs/opendox-carve-manifest.yaml:1807-1810 RULED OQ-F) and 5 "
        "CLASS-BOUNDARY (app.js:43 -> views/gate.js, app.js:45 -> "
        ...
        "sites); the 5 class-boundary sites are the same shape S3's view "
        "registry and S5's gate-loop contribution close, per § 4.1/§ 5, not "
        "S2. This assertion therefore stays red after S2 alone -- flagged "
        "here rather than narrowed to fit the note's own summary."
    ),
)
